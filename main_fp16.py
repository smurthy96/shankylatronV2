...
if __name__ == "__main__":
    ...
    model = Translatotron(auxiliary_decoder_output_dim_source=256,
                          auxiliary_decoder_output_dim_target=5356).to(device).half()

    mse_loss = nn.MSELoss().to(device).half()
    ce_loss = nn.CrossEntropyLoss().to(device).half()
    ...
    class FP16MelSpectrogram(T.MelSpectrogram):
        def __init__(self, *args, **kwargs):
            super().__init__(*args, **kwargs)

        def forward(self, waveform):
            # Ensure waveform is in float16
            waveform = waveform.half()
            # Convert self.fb to float16 if it is not already
            self.mel_scale.fb = self.mel_scale.fb.to(waveform.device).half()
            # Call the original forward
            return super().forward(waveform)
    mel_converter = FP16MelSpectrogram(sample_rate=16000, n_fft=1024, hop_length=256, n_mels=80).to(device).half()
    #mel_converter.mel_scale.fb = mel_converter.mel_scale.fb.to(device).half()
    ...
    def train_one_epoch(model, loader, optimizer, epoch):
        ...
        for batch_idx, (source_audio, target_audio, source_text, target_text) in enumerate(loader):
            # Move data to device
            source_audio, target_audio = source_audio.to(device), target_audio.to(device).half()
            source_text, target_text = source_text.to(device), target_text.to(device).half()
            ...
            mask = (target_mel != 0).float().half()  # Assume padding is 0
